...
Actually we use dynamic array frequently, like `list` in python, `vector` in c++, `ArrayList/List` in Java
...
`insert(A, x)`: check before insert, if full, create new one and copy all of them/concatenate if use other way (such as index attribute)
