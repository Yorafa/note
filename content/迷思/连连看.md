...
说干就干，首先不能做一些明显违法的行为比如改数据一类的。那只能模拟自己玩了。这时候我就想到python的[pyautogui](https://pyautogui.readthedocs.io/en/latest/)库. 这很符合我们的需要，接下来我要做的事情就是直接把puzzle的gameboard数组化，然后跑解就可以了！
...
原本写了bfs，但是跑起来太慢了，目测时间复杂度至少要$2^{30+}$ 。于是还是决定暴力做。虽然会fail, 但效率上实在高上不少。
